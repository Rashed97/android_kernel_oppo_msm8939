 ...
 * 2.0        2015-04-13  hantong   modify as platform device to support
 *                                  different configuration in dts
 */
...
static int get_hw_operator_version(struct devinfo_data *devinfo_data)
{
	...
	devinfo_data->hw_id1_gpio = of_get_named_gpio(np, "hw,operator-gpio1",
			0);
	if (devinfo_data->hw_id1_gpio < 0) {
		pr_info("devinfo_data->hw_id1_gpio not specified\n");
	}

	devinfo_data->hw_id2_gpio = of_get_named_gpio(np, "hw,operator-gpio2",
			0);
	if (devinfo_data->hw_id2_gpio < 0) {
		pr_info("devinfo_data->hw_id2_gpio not specified\n");
	}

	devinfo_data->hw_id3_gpio = of_get_named_gpio(np, "hw,operator-gpio3",
			0);
	if (devinfo_data->hw_id3_gpio < 0) {
		pr_info("devinfo_data->hw_id3_gpio not specified\n");
	}
...
static void sub_mainboard_verify(struct devinfo_data *devinfo_data)
{
	...
	if (devinfo_data->sub_hw_id1 < 0) {
		pr_info("devinfo_data->sub_hw_id1 not specified\n");
	}

	devinfo_data->sub_hw_id2 = of_get_named_gpio(np, "hw,sub_hwid_2", 0);
	if (devinfo_data->sub_hw_id2 < 0) {
		pr_info("devinfo_data->sub_hw_id2 not specified\n");
	}
