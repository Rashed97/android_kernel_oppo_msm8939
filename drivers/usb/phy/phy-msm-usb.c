...
static enum hrtimer_restart msm_otg_timer_func(struct hrtimer *hrtimer)
{
	...
	#ifndef CONFIG_MACH_OPPO /*dengnw@BSP.drv add QCOM patch for OTG 20150115*/
...
#ifdef CONFIG_MACH_OPPO /*dengnw@BSP.drv add QCOM patch for OTG 20150115*/
...
static int msm_otg_set_suspend(struct usb_phy *phy, int suspend)
{
	...
	if (suspend) {
		...
		case OTG_STATE_A_HOST:
			...
			clear_bit(A_BUS_REQ, &motg->inputs);
			#if 0//def CONFIG_MACH_OPPO /*dengnw@BSP.drv add QCOM patch for OTG 20150115*/
			mutex_lock(motg->inputbits_mutex);
			#endif
			
			#ifndef CONFIG_MACH_OPPO /*liaofuchun@bsp.drv add for otg switch in 20150216*/
